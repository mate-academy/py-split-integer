...
def test_should_split_into_equal_parts_when_value_divisible_by_parts() -> None:
    assert split_integer(6, 2)[0] == split_integer(6, 2)[1], \
        ("Function 'split_integer' should split value "
         "into equal parts when value divisible by parts")


def test_should_return_part_equals_to_value_when_split_into_one_part() -> None:
    assert split_integer(8, 1) == [8], \
        ("The result list should contain one "
         "element equals to value when split into one part")
