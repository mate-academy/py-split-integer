...
def test_sum_of_the_parts_should_be_equal_to_value(
    ...
    assert result == expected_sum, \
        f"Expected sum to be {expected_sum}, but got {result}"
...
def test_parts_should_be_sorted_when_they_are_not_equal(
    ...
    assert parts == sorted(parts), f"Expected sorted parts - got {parts}"
...
def test_should_add_zeros_when_value_is_less_than_number_of_parts(
    ...
    assert len(result) == number_of_parts, (
        f"Expected {number_of_parts} parts - got {len(result)} parts"
    )
    assert result.count(0) == number_of_parts - value, (
        f"Expected {number_of_parts - value} zeros - got {result.count(0)}"
    )
