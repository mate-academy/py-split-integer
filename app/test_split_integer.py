...
def test_sum_of_the_parts_should_be_equal_to_value() -> None:
    ...
    assert sum(result) == value, "The sum of parts is not equal to the original value"
...
def test_parts_should_be_sorted_when_they_are_not_equal() -> None:
    ...
    assert max(result) - min(result) <= 1, "Difference between max and min parts is greater than 1"
...
def test_should_add_zeros_when_value_is_less_than_number_of_parts() -> None:
    ...
    assert len(result) == number_of_parts, (
        f"Result list length ({len(result)}) does not match the number of parts ({number_of_parts})"
    )
    assert result == sorted(result), f"Result list is not sorted: {result}"
